...
module.exports = (config) => {
  ...
  app.get('/api/ping', (req, res) => {
    req.session.touch(req.session.id, req.session, null);
    res.sendStatus(200);
  });
  ...
  app.get('/api/files/:uuid', auth.requireAuth, files.download)
  app.post('/api/files/', auth.requireAuth, upload.any(), files.upload);
